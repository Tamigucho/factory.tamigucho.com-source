...
//  }, []);
      ...
      const currentItems = creaturesData.slice(indexOfFirstItem, indexOfLastItem);
      const totalPages = Math.ceil(creaturesData.length / itemsPerPage);
      const [dropdownOpen, setDropdownOpen] = useState(false);
      const toggle = () => setDropdownOpen(prevState => !prevState);
      const currentYear = Math.max(...creatures.map(pokemon => creature.year));
      const yearsPassed = currentYear - 2010;
      const tooltip = `There are ~${creaturesData.length} Pokémons in existence from 2010 to ${currentYear} (${yearsPassed} years)`;
    return (
      <div className="creatures-page">
        <h2>Creatures Dex <span className="badge bg-secondary" title={tooltip}>{creaturesData.length}</span></h2>
        <div className="pokemon-list">
        {currentItems.map((pokemon, index) => (
  <CreatureItem key={index} {...pokemon} />
))}
        </div>
